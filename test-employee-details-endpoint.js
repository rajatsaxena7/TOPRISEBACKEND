...
async function testEmployeeDetailsEndpoint() {
    console.log('🔍 Testing Employee Details Endpoint with Dealer Data Population');
    console.log('================================================================\n');

    console.log(`📋 Testing with employee ID: ${EMPLOYEE_ID}`);
    console.log(`🌐 Endpoint: ${BASE_URL}/api/users/employee/${EMPLOYEE_ID}\n`);

    try {
        const response = await axios.get(`${BASE_URL}/api/users/employee/${EMPLOYEE_ID}`, {
            headers: {
...
async function testMultipleEmployeeDetails() {
    ...
    for (const employeeId of testEmployeeIds) {
        ...
        try {
            const response = await axios.get(`${BASE_URL}/api/users/employee/${employeeId}`, {
                headers: {
                    'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE', // Replace with actual token
                    'Content-Type': 'application/json'
                },
...
async function testPerformance() {
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        try {
            await axios.get(`${BASE_URL}/api/users/employee/${EMPLOYEE_ID}`, {
                headers: {
                    'Authorization': 'Bearer YOUR_JWT_TOKEN_HERE', // Replace with actual token
                    'Content-Type': 'application/json'
                },
...
// Compare with the other employee endpoint
async function compareEndpoints() {
    console.log('\n🔄 Comparing Employee Endpoints');
    console.log('================================\n');

    const employeeId = EMPLOYEE_ID;

    console.log('📋 Testing both endpoints with the same employee ID...\n');

    // Test getEmployeeDetails endpoint
    console.log('1️⃣ Testing getEmployeeDetails endpoint:');
    console.log('─'.repeat(50));

    try {
        const response1 = await axios.get(`${BASE_URL}/api/users/employee/${employeeId}`, {
            headers: {
        ...
        if (response1.status === 200 && response1.data.success) {
            ...
            console.log(`✅ getEmployeeDetails: Found ${data1.First_name}`);
            console.log(`   - Dealers: ${data1.assigned_dealers?.length || 0}`);
            console.log(`   - Regions: ${data1.assigned_regions?.length || 0}`);
        } else {
            console.log(`❌ getEmployeeDetails: ${response1.data.message || 'Unknown error'}`);
        }
    } catch (error) {
        console.log(`❌ getEmployeeDetails: ${error.response?.data?.message || error.message}`);
    }

    // Test getEmployeeById endpoint
    console.log('\n2️⃣ Testing getEmployeeById endpoint:');
    console.log('─'.repeat(50));

    try {
        const response2 = await axios.get(`${BASE_URL}/api/users/employee/get-by-id`, {
        ...
        if (response2.status === 200 && response2.data.success) {
            ...
            console.log(`✅ getEmployeeById: Found ${data2.First_name}`);
            console.log(`   - Dealers: ${data2.assigned_dealers?.length || 0}`);
            console.log(`   - Regions: ${data2.assigned_regions?.length || 0}`);
        } else {
            console.log(`❌ getEmployeeById: ${response2.data.message || 'Unknown error'}`);
        }
    } catch (error) {
        console.log(`❌ getEmployeeById: ${error.response?.data?.message || error.message}`);
    }

    console.log('\n📝 Note: Both endpoints should return similar data with dealer information populated.');
}

// Main execution
async function runTests() {
    console.log('🎯 Employee Details Endpoint Test Suite');
    console.log('=======================================\n');
    ...
    try {
        await testEmployeeDetailsEndpoint();
        await testMultipleEmployeeDetails();
        ...
        console.log('   6. Compare with the getEmployeeById endpoint');
...
module.exports = {
    testEmployeeDetailsEndpoint,
    testMultipleEmployeeDetails,
    testWithoutAuth,
    testPerformance,
    compareEndpoints,
    runTests
