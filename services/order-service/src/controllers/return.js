...
const PRODUCT_SERVICE_URL = process.env.PRODUCT_SERVICE_URL || "http://product-service:5002/api/products";
...
exports.createReturnRequest = async (req, res) => {
  try {
    ...
    if (!orderId || !sku || !returnReason || !customerId) {
      return sendError(res, "Missing required fields: orderId, sku, returnReason, customerId");
    }

    // Check if return already exists for this order and SKU
    const existingReturn = await Return.findOne({ orderId, sku });
    if (existingReturn) {
      return sendError(res, "Return request already exists for this order and SKU");
    }
    ...
    const returnRequest = await Return.create({
      ...
      originalDeliveryDate: order.skus.find(s => s.sku === sku)?.tracking_info?.timestamps?.deliveredAt,
    ...
    return sendSuccess(res, returnRequest, "Return request created successfully");
  } catch (error) {
...
exports.validateReturnRequest = async (req, res) => {
  try {
    ...
    // Validate return eligibility
    const eligibilityResult = await validateReturnEligibility(order, returnRequest.sku);
    ...
    returnRequest.returnStatus = eligibilityResult.isEligible ? "Validated" : "Requested";
    ...
    await createUnicastOrMulticastNotificationUtilityFunction(
      ...
      `Your return request has been ${eligibilityResult.isEligible ? 'approved' : 'rejected'}: ${eligibilityResult.reason}`,
      "",
      "",
      "Return",
      { returnId: returnRequest._id },
    ...
    return sendSuccess(res, returnRequest, "Return request validated successfully");
  } catch (error) {
...
exports.schedulePickup = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Validated") {
      return sendError(res, "Return request must be validated before scheduling pickup");
    }

    // Create pickup request with logistics partner
    const pickupRequest = await createLogisticsPickupRequest(returnRequest, scheduledDate, pickupAddress);
...
exports.completePickup = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Pickup_Scheduled") {
      return sendError(res, "Return request must be in Pickup_Scheduled status");
    }
...
exports.startInspection = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Pickup_Completed") {
      return sendError(res, "Return request must be in Pickup_Completed status");
    }
...
exports.completeInspection = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Under_Inspection") {
      return sendError(res, "Return request must be in Under_Inspection status");
    }
    ...
    // Send notification to customer
    const notificationTitle = isApproved ? "Return Approved" : "Return Rejected";
...
exports.processRefund = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Approved") {
      return sendError(res, "Return request must be approved before processing refund");
    }

    // Process refund with payment gateway
    const refundResult = await processRefundPayment(returnRequest, refundMethod);
    
    if (!refundResult.success) {
      return sendError(res, `Refund processing failed: ${refundResult.message}`);
    }
...
exports.completeReturn = async (req, res) => {
  try {
    ...
    if (returnRequest.returnStatus !== "Refund_Processed") {
      return sendError(res, "Return request must have refund processed before completion");
    }
...
exports.getReturnRequest = async (req, res) => {
  try {
    ...
    return sendSuccess(res, returnRequest, "Return request fetched successfully");
  } catch (error) {
          ...
          averageProcessingTime: {
            $avg: {
              $cond: [
                ...
                { $subtract: ['$timestamps.completedAt', '$timestamps.requestedAt'] },
                null
              ]
            }
          },
...
async function validateReturnEligibility(order, sku) {
  try {
    ...
    try {
      const productResponse = await axios.get(`${PRODUCT_SERVICE_URL}/sku/${sku}`);
      if (productResponse.data?.success) {
 ...
 */
async function createLogisticsPickupRequest(returnRequest, scheduledDate, pickupAddress) {
  try {
    // This would integrate with your logistics partner API
    // For now, we'll create a mock pickup request
    const pickupId = `PICKUP_${Date.now()}`;
