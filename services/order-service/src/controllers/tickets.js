...
exports.createTicket = async (req, res) => {
    try {
        const { userRef, order_id, updated_by = null, description, ticketType } = req.body;
...
exports.updateTicketStatus = async (req, res) => {
    try {
        ...
        if (status === "Resolved" || status === "Closed") {
            const user = await axios.put(`${USER_SERVICE_URL}/api/users/remove-support/${updatedTicket._id}/${updatedTicket.assigned_to}`);
...
exports.getTicketByInvolvedUserRef = async (req, res) => {
    try {
        ...
        logger.info(`Ticket retrieved successfully for userRef: ${involved_userId}`);
        sendSuccess(res, ticket, 'Ticket retrieved successfully');
    } catch (error) {
        logger.error(`Error retrieving ticket by userRef: ${error.message}`);
        sendError(res, error);
