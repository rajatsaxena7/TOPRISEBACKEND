...
cron.schedule('* * * * *', async () => {
    ...
    try {
        ...
        const userData = await axios.get(`${USER_SERVICE_URL}/api/users/`);
        const userList = userData.data.data;
        const agents = userList.filter(user => user.role === 'Customer-Support');
        ...
        for (const ticket of unassignedTickets) {
            try {
                ...
                await axios.put(`${USER_SERVICE_URL}/api/users/assign-support/${ticket._id}/${agent._id}`, );
